...
LangChain의 `BaseRateLimiter`와 호환되는 Redis 기반 RateLimiter입니다.
**분당/초당 요청 수(RPM/RPS)** 제한 기능을 지원하며, 분산 환경에서도 안정적으로 사용이 가능합니다.

## 주요 기능

- **Redis 지원**: Local Redis, AWS ElastiCache와 호환됩니다.
...
- `requests_per_second`: 1초 동안 몇 번까지 요청을 보낼 수 있는지 설정합니다. 예: 5라면 초당 최대 5번까지 호출 가능합니다. 분당 요청 수(RPM)를 설정하려면 원하는 RPM 값을 60으로 나누어 입력하세요 (예: 0.5로 설정하면 30 RPM과 동일).
